...
- Current failure point: the widget renders the three `FilterChip`s immediately, so the collapsed, expansion, and zero-budget tests all fail on the hidden-filter expectation before the intended collapsed-summary behavior can be observed.
