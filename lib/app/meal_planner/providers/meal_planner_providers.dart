...
class MealTypes extends _$MealTypes {
  ...
  Future<void> deleteMealType(String id) async {
    ...
    final type = currentTypes.firstWhereOrNull((t) => t.id == id);
    if (type != null) {
      if (type.isBuiltIn) {
        // Safe check: mark built-in as deleted
        await service.saveMealType(type.copyWith(isDeleted: true));
      } else {
        await service.deleteMealType(id);
      }
    } else {
      // If it's a built-in not yet in Firestore, we can write it as isDeleted: true
      final defaultsList = MealType.defaults;
      final defaultType = defaultsList.firstWhereOrNull((t) => t.id == id);
      if (defaultType != null) {
        await service.saveMealType(defaultType.copyWith(isDeleted: true));
      }
    }
  }
...
List<MealType> activeMealTypes(Ref ref) {
  final customTypes = ref.watch(mealTypesProvider).value ?? [];
  final defaultList = MealType.defaults;
  final map = {for (final d in defaultList) d.id: d};

  for (final custom in customTypes) {
    if (custom.isDeleted) {
      map.remove(custom.id);
    } else {
      map[custom.id] = custom;
    }
  }

  final list = map.values.toList()..sort((a, b) => a.sortOrder.compareTo(b.sortOrder));
  return list;
}
