...
import '../../../models/suggested_reply.dart';
...
class AiChatPanelState extends ConsumerState<AiChatPanel> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  // Suggestions affordance state
  bool _hasUsedSuggestion = false;
  bool _showSuggestionsOverlay = true;
  ...
  Future<void> sendMessage([String? manualText]) async {
    ...
    try {
      ...
      if (mounted) {
        showKipiSnackBar(
          context,
          message: l10n?.aiError ?? 'Erro ao processar. Verifique sua conexão.',
          type: SnackBarType.error,
        );
      }
    } finally {
  ...
  /// Called when the user taps a suggested reply chip. Shows a one-time
  /// micro-celebration on first use to acknowledge the discovery.
  void onSuggestionTapped(String prompt) {
    unawaited(HapticFeedback.selectionClick());
    // Collapse the keyboard-overlay bar once the user picks a suggestion
    // so the full input area comes back to focus.
    setState(() => _showSuggestionsOverlay = false);
    if (!_hasUsedSuggestion && mounted) {
      setState(() => _hasUsedSuggestion = true);
      showKipiSnackBar(
        context,
        message: 'A IA está aprendendo suas preferências!',
  ...
  Widget build(BuildContext context) {
    ...
    if (activeSessionId != _lastSessionId) {
    ...
    // Extract suggested replies from the last assistant message to show as a
    // pinned bar above the input when the keyboard is visible.
    final messages = chatState.value ?? const <ChatMessage>[];
    final lastMsg = messages.isNotEmpty ? messages.last : null;
    final pinnedSuggestions =
        lastMsg != null &&
                lastMsg.role != 'user' &&
                !isStreaming &&
                !isThinking &&
                !(lastMsg.isTeaser)
            ? lastMsg.suggestedReplies
            : null;
    final hasPinnedSuggestions =
        pinnedSuggestions != null && pinnedSuggestions.isNotEmpty;

    // Re-show the overlay whenever a fresh batch of suggestions lands so the
    // user always gets a chance to see them after the keyboard dismisses.
    ref.listen(chatSessionProvider(widget.listId, activeSessionId), (prev, next) {
      final prevLast = prev?.value?.isNotEmpty == true ? prev!.value!.last : null;
      final nextLast = next.value?.isNotEmpty == true ? next.value!.last : null;
      if (nextLast != null &&
          nextLast.id != prevLast?.id &&
          nextLast.role != 'user' &&
          nextLast.suggestedReplies != null &&
          nextLast.suggestedReplies!.isNotEmpty) {
        setState(() => _showSuggestionsOverlay = true);
      }
    });
      ...
      children: [
                ...
                child: chatState.when(
                  data: (allMessages) {
                    ...
                    if (allMessages.isEmpty) {
                      return _buildEmptyState(l10n, theme, key: const ValueKey('chat_empty_state'));
                    }
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if (msgIndex < messages.length) {
                          ...
                          final hasAnimated = _animatedMessageIds.contains(message.id);
                          if (!hasAnimated) {
                            _animatedMessageIds.add(message.id);
                          }

                          final bubble = _GroupChatBubble(
                            message: message,
                          ...
                          final animatedBubble = hasAnimated
                              ? bubble
                              : bubble.animate().fadeIn(
                  ...
                  loading: () => _buildSkeleton(theme, l10n, key: const ValueKey('chat_skeleton')),
                  error: (e, _) => Center(
                    key: const ValueKey('chat_error_state'),
                    child: Padding(
              ...
              if (_showScrollFAB)
                Positioned(
                  ...
                  child: FloatingActionButton.small(
                    ...
                    child: const Icon(PhosphorIconsRegular.arrowDown),
        ...
        Column(
          ...
          children: [
            ...
            // Pinned suggestions overlay: shown when keyboard is up so chips
            // don't get scrolled out of view. Also shown when the user toggles
            // the lightbulb button in the input bar.
            if (hasPinnedSuggestions && _isKeyboardVisible && _showSuggestionsOverlay)
              _PinnedSuggestionsBar(
                replies: pinnedSuggestions!,
                onSelected: onSuggestionTapped,
                onDismiss: () => setState(() => _showSuggestionsOverlay = false),
              ),
            _buildInput(context, l10n, theme, canSend, isThinking, isInputBlocked,
                hasSuggestions: hasPinnedSuggestions),
          ],
        ),
      ],
    );
  ...
  Widget _buildSenderName(ChatMessage message, ThemeData theme, bool isFirstInGroup) {
    ...
    final displayName = isGroup ? (message.senderName ?? aiConfig.name) : aiConfig.name;
    return Padding(
  ...
  Widget _buildEmptyState(AppLocalizations l10n, ThemeData theme, {Key? key}) {
    final icons = [PhosphorIconsRegular.shoppingCart, PhosphorIconsRegular.forkKnife, PhosphorIconsRegular.piggyBank, PhosphorIconsRegular.squaresFour];
    final titles = ['Montar lista', 'Receitas', 'Economizar', 'Organizar'];
    final subs = ['Lista completa da semana', 'Com o que você já tem', 'Gastar menos', 'Por corredor'];
    final prompts = [
      'Monte uma lista de compras completa para a semana',
      'Sugira receitas com os itens da minha lista atual',
      'Como posso economizar mais nas minhas compras?',
      'Organize minha lista por categorias do supermercado',
    ];
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Icon(icons[i], size: 20, color: theme.colorScheme.primary),
                 ...
                 .slideY(begin: 0.1, end: 0, duration: 200.ms, delay: (i * 60).ms),
              );
            }),
          ),
        ),
  ...
  Widget _buildSkeleton(ThemeData theme, AppLocalizations l10n, {Key? key}) {
    return Skeletonizer(
      key: key,
      child: Column(
        children: [
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(PhosphorIconsRegular.sparkle, size: 14, color: theme.colorScheme.primary),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(PhosphorIconsRegular.sparkle, size: 16, color: theme.colorScheme.primary),
              const SizedBox(width: Spacing.xxs),
              Expanded(
                child: Text(
                  remaining > 0 ? l10n.aiEnergyLow : l10n.aiUnlockUnlimited,
                  style: theme.textTheme.labelMedium?.copyWith(
          ...
          Text(
            remaining > 0
                ? l10n.aiRemainingFreeToday(remaining)
                : l10n.aiSubscribeUnlimited,
            style: theme.textTheme.bodySmall?.copyWith(
              color: theme.colorScheme.onSurfaceVariant,
            ),
          ),
  ...
  Widget _buildInput(BuildContext context, AppLocalizations l10n, ThemeData theme, bool canSend, bool isThinking, bool inputBlocked, {bool hasSuggestions = false}) {
    final showPlus = widget.listId != null;
    final isKeyboardOpen = _isKeyboardVisible;

    // IMPORTANT: all Row children use ternary (not `if`) so child COUNT never
    // changes. Flutter reconciles by index — stable positions keep the TextField
    // in the same slot across rebuilds, preserving the TextInputConnection.
    final inputWidget = ValueListenableBuilder<TextEditingValue>(
      ...
      builder: (context, value, child) {
        ...
        return Row(
          ...
          children: [
            // Position 0: plus button outside pill (keyboard open) — or empty.
            ...
            // Position 1: gap after plus — or empty.
            if (showPlus && isKeyboardOpen)
              const SizedBox(width: 8)
            else
              const SizedBox.shrink(),
            // Position 2: input pill — always Expanded so index never shifts.
            Expanded(
              child: AnimatedContainer(
                duration: DurationTokens.fast,
                decoration: BoxDecoration(
                  color: _isListening
                      ? theme.colorScheme.primaryContainer.withAlpha((0.35 * 255).toInt())
                      : inputBlocked
                          ? theme.colorScheme.surfaceContainerHigh.withAlpha((0.5 * 255).toInt())
                          : theme.colorScheme.surfaceContainerHigh,
                  borderRadius: BorderRadius.circular(28),
                  border: Border.all(
                    color: _focusNode.hasFocus
                        ? theme.colorScheme.primary.withAlpha((0.4 * 255).toInt())
                        : theme.colorScheme.outlineVariant.withAlpha((0.2 * 255).toInt()),
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: [
                    // Position 0: left icon — plus inside pill, wave, or empty.
                    if (!isKeyboardOpen && showPlus)
                      Padding(
                      ...
                      Padding(
                        ...
                        child: _buildWave(theme),
                      )
                    else
                      const SizedBox.shrink(),
                    // Position 1: TextField — always here, never moves.
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintStyle: theme.textTheme.bodyMedium?.copyWith(
                            color: inputBlocked
                                ...
                                : _isListening
                                    ? theme.colorScheme.primary
                                    : theme.colorScheme.onSurfaceVariant.withAlpha((0.45 * 255).toInt()),
                          ),
                          filled: false,
                    ...
                    // Position 2: suggestions lightbulb — or empty.
                    if (hasSuggestions && isKeyboardOpen)
                      Tooltip(
                        message: 'Sugestões da IA',
                        child: IconButton(
                          key: const ValueKey('chat_suggestions_toggle'),
                          icon: const Icon(PhosphorIconsRegular.lightbulb, size: 20),
                          color: _showSuggestionsOverlay
                              ? theme.colorScheme.primary
                              : theme.colorScheme.onSurfaceVariant.withAlpha(160),
                          padding: EdgeInsets.zero,
                          constraints: const BoxConstraints(minWidth: 36, minHeight: 36),
                          onPressed: () => setState(
                            () => _showSuggestionsOverlay = !_showSuggestionsOverlay,
                          ),
                        ),
                      )
                    else
                      const SizedBox.shrink(),
                    // Position 3: action button (stop/send/mic) — always here.
                    Padding(
    ...
    // Single structural path — AnimatedContainer morphs appearance without
    // rebuilding the widget tree, so the TextField is never destroyed and its
    // TextInputConnection is never closed by a layout switch.
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        height: 38,
        decoration: BoxDecoration(
          color: onPressed == null ? background.withAlpha(120) : background,
          shape: BoxShape.circle,
        ),
        child: Icon(icon, size: 20, color: onPressed == null ? color.withAlpha(120) : color),
      ),
    );
  }
}
...
/// Pinned horizontal chips bar rendered above the input when the keyboard is
/// up. Keeps suggested replies reachable even after the user taps the text
/// field and the chat list scrolls the last-message chips off screen.
class _PinnedSuggestionsBar extends StatelessWidget {
  const _PinnedSuggestionsBar({
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    // Limit to 3 chips when keyboard is up to avoid stacking too much chrome.
    final visible = replies.take(3).toList();

    return Container(
      color: theme.colorScheme.surface,
      padding: const EdgeInsets.fromLTRB(Spacing.sm, Spacing.xxs, Spacing.xs, Spacing.xxs),
      child: Row(
        children: [
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  for (final (index, reply) in visible.indexed) ...[
                    if (index > 0) const SizedBox(width: Spacing.xs),
                    ActionChip(
                      avatar: Icon(
                        _pinnedSuggestionIcon(reply.icon),
                        size: 14,
                        color: theme.colorScheme.primary,
                      ),
                      label: Text(
                      ...
                      onPressed: () => onSelected(reply.prompt),
                      side: BorderSide(
  ...
  IconData _pinnedSuggestionIcon(String icon) {
    return switch (icon) {
      'add' || 'add_circle' || 'add_shopping_cart' => PhosphorIconsRegular.plusCircle,
      'delete' || 'remove' => PhosphorIconsRegular.trash,
      'recipe' || 'restaurant' => PhosphorIconsRegular.forkKnife,
      'report' || 'pdf' => PhosphorIconsRegular.filePdf,
      'shopping_cart' || 'cart' => PhosphorIconsRegular.shoppingCart,
      'check' || 'done' => PhosphorIconsRegular.checkCircle,
      'savings' => PhosphorIconsRegular.piggyBank,
      _ => PhosphorIconsRegular.arrowUpRight,
    };
  }
}
                              ...
                              children: [
                                             ...
                                             AnimatedSwitcher(
                                               ...
                                               child: Row(
                                                 ...
                                                 children: [
                                                   ...
                                                   const SizedBox(width: 8),
                                                   const RepaintBoundary(child: AnimatedTypingDots()),
                                                 ],
                                               ),
                                             ),
                                           ],
                                          ...
                                          child: isUser
                                              ? Container(
                                                  ...
                                                  decoration: BoxDecoration(
                                                    gradient: LinearGradient(
                                                      colors: [
                                                        theme.colorScheme.primary,
                                                        theme.colorScheme.primary.withAlpha((0.88 * 255).toInt()),
                                                      ],
                                              ...
                                              : Container(
                                                  ...
                                                  decoration: BoxDecoration(
                                                    ...
                                                    border: Border.all(
                                                      color: theme.colorScheme.outlineVariant.withAlpha((0.35 * 255).toInt()),
                                                      width: 1,
                                                    ),
                                                  ),
                                                  child: _buildMarkdownContent(
                                    ...
                                    if (!isUser && widget.message.isError) ...[
                                      ...
                                      FilledButton.icon(
                                        ...
                                        icon: const Icon(PhosphorIconsRegular.arrowCounterClockwise, size: 16),
                                        label: Text(l10n.retry),
                                        style: FilledButton.styleFrom(
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  _FeedbackButton(
                    icon: widget.message.feedback == 1 ? PhosphorIconsRegular.thumbsUp : PhosphorIconsRegular.thumbsUp,
                  ...
                  const SizedBox(width: 8),
                  _FeedbackButton(
                    icon: widget.message.feedback == -1 ? PhosphorIconsRegular.thumbsDown : PhosphorIconsRegular.thumbsDown,
                    isSelected: widget.message.feedback == -1,
                    onTap: () {
                      unawaited(HapticFeedback.lightImpact());
                      final sessionId = ref.read(activeChatSessionIdProvider(widget.listId));
                ...
                child: Row(
                  children: widget.message.suggestedReplies!.asMap().entries.map((entry) {
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        onTap: () {
                          // Route through onSuggestionTapped for celebration
                          // and overlay tracking instead of calling sendMessage
                          // directly — haptic is handled there.
                          final panelState = context.findAncestorStateOfType<AiChatPanelState>();
                          panelState?.onSuggestionTapped(reply.prompt);
                        },
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color: Theme.of(context).colorScheme.primary.withAlpha((0.25 * 255).toInt()),
                            ),
                          ),
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                        ...
                        duration: 200.ms,
                        delay: (index * 60).ms,
                      ).slideX(begin: 0.1, end: 0, duration: 200.ms, delay: (index * 60).ms),
                    );
                  }).toList(),
                ),
              ),
...
IconData _iconFromKey(String? key) {
  switch (key) {
    case 'add_shopping_cart':
      return PhosphorIconsRegular.shoppingCartSimple;
    case 'savings':
      return PhosphorIconsRegular.piggyBank;
    case 'restaurant':
        ...
        child: Row(
          ...
          children: [
            Container(
             ...
             .scale(begin: const Offset(0.85, 0.85), end: const Offset(1, 1), duration: 800.ms, curve: Curves.easeInOut),  // ignore: prefer_int_literals
            if (isThinking && (activityDescription != null || fallbackText != null))
              AnimatedSwitcher(
                duration: const Duration(milliseconds: 300),
                ...
                child: Text(
                  activityDescription ?? fallbackText ?? 'Pensando...',
...
class _ActionButtonState extends State<_ActionButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final icon = _isSuccess ? PhosphorIconsRegular.check : widget.icon;
...
class _AgentActionStepsList extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_AgentActionStepsList> createState() => _AgentActionStepsListState();
}

class _AgentActionStepsListState extends ConsumerState<_AgentActionStepsList> {
  ...
  Widget build(BuildContext context) {
    ...
    final isLocked = steps.any((s) => s.status == AgentStepStatus.requiresUnlock);
    ...
    if (isRunning) {
      ...
      statusText = 'Falha ao executar ações';
      statusIcon = Icon(PhosphorIconsRegular.warningCircle, size: 14, color: theme.colorScheme.error);
      textColor = theme.colorScheme.error;
    } else if (isLocked) {
      statusText = 'Ações requerem Premium';
      statusIcon = const Icon(PhosphorIconsRegular.crown, size: 14, color: Colors.amber);
      textColor = Colors.amber.shade800;
    } else {
      statusText = steps.length == 1 ? '1 ação realizada' : '${steps.length} ações realizadas';
      statusIcon = Icon(PhosphorIconsRegular.checkCircle, size: 14, color: theme.colorScheme.primary);
    ...
    return Container(
      decoration: BoxDecoration(
        color: theme.colorScheme.surfaceContainerHigh.withAlpha((0.5 * 255).toInt()),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: theme.colorScheme.outlineVariant.withAlpha((0.3 * 255).toInt()),
        ),
                        ...
                        Padding(
                          ...
                          child: Align(
                            ...
                            child: TextButton.icon(
                              ...
                              label: const Text('Desfazer alterações', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                              style: TextButton.styleFrom(
  ...
  Widget build(BuildContext context) {
    ...
    switch (status) {
      case AgentStepStatus.pending:
        leading = Icon(
          isPremiumTool ? PhosphorIconsRegular.crown : PhosphorIconsRegular.circle,
          size: 14,
          color: isPremiumTool 
    ...
    if (result.containsKey('itemId') && !result.containsKey('previousState')) {
      ...
      try {
        ...
        details.add(Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Icon(PhosphorIconsRegular.info, size: 12, color: theme.colorScheme.onSurfaceVariant),
            const SizedBox(width: 4),
            Expanded(
              child: Text(
      ...
      try {
        ...
        details.add(Row(
          children: [
            Icon(PhosphorIconsRegular.listChecks, size: 12, color: theme.colorScheme.onSurfaceVariant),
            const SizedBox(width: 4),
            Text(
      ...
      details.add(Row(
        children: [
          Icon(PhosphorIconsRegular.wallet, size: 12, color: theme.colorScheme.onSurfaceVariant),
          const SizedBox(width: 4),
          Text(
