...
const openPaywallTool = AgentTool(
  name: 'open_paywall',
  description:
      'Abre a tela de assinatura Premium do aplicativo para que o usuário possa ver os planos e benefícios.',
);

const requestAppReviewTool = AgentTool(
  name: 'request_app_review',
  description:
      'Solicita que o usuário avalie o aplicativo na loja (Play Store/App Store). '
...
const promptAppUpdateTool = AgentTool(
  ...
  description:
      'Verifica e solicita que o usuário atualize o aplicativo para a versão mais recente.',
);

const createShareCodeTool = AgentTool(
  name: 'create_share_code',
  description:
      'Cria um código de compartilhamento para que o usuário possa convidar outra pessoa '
...
const generateReferralLinkTool = AgentTool(
  ...
  description:
      'Gera um link de convite personalizado. Se o convidado baixar o app, '
...
const importSharedListTool = AgentTool(
  ...
  description:
      'Importa uma lista compartilhada usando um código de 8 caracteres fornecido pelo usuário.',
  parameters: [
    AgentToolParameter(
      name: 'code',
      type: 'string',
